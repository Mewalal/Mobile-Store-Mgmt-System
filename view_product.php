...
<section class="py-5 bg-light">
    <div class="container px-4 px-lg-5 mt-5">
        <h2 class="fw-bolder mb-4">recommendation</h2>
        <div class="row gx-4 gx-lg-5 row-cols-1 row-cols-md-3 row-cols-xl-4 justify-content-center">
        <?php 
            $products = $conn->query("SELECT p.*,b.name as bname FROM `products` p inner join brands b on p.brand_id = b.id where p.status = 1 and (p.category_id = '{$category_id}' or p.sub_category_id = '{$sub_category_id}') and p.id !='{$id}' order by rand() limit 5 ");
        ...
        $('#add-cart').submit(function(e){
            ...
            $.ajax({
                ...
                success:function(resp){
                    ...
                    // if{resp.cart_count => 3
                    //     alert_toast("you can't buy more than 3 product")
                    // }
                    else{
                        console.log(resp)
                        alert_toast("an error occured",'error')
